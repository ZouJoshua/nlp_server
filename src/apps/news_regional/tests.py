...
NLP_REGIONAL_FILE_PATH = r'/home/zoushuai/algoproject/nlp_server/src/data/news_regional/india_names2regions.json'
...
class TestPredict(TestCase):
    ...
    def test_time_use(self):
        ...
        s1 = time.time()
        print(">>>>>>>>>>测试函数：{}".format(inspect.stack()[0].function))
    ...
    def test_split_text_find_time_use(self):
        s = time.time()
        text = self.content + '.' + self.title
        out_count = pred._match_detail_regional(text, regional_map)
        s1 = time.time()
        print(">>>>>>>>>>测试函数：{}".format(inspect.stack()[0].function))
        print(">>>>>>>>>>查找地域耗时： {}".format(s1 - s))
        regional_ct = pred._count_regional(out_count, regional_map)
